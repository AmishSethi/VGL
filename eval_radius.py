...
from models_radius import DiT_models_continuous as DiT_models
...
def load_model(config, device):
    ...
    # For SongUNet, detect channels from checkpoint for backwards compatibility
        ...
        for key in state_dict.keys():
            if 'enc.' in key and 'conv.weight' in key:
                ...
                in_channels = checkpoint_channels  # Use the checkpoint's channel configuration
                out_channels = checkpoint_channels  # Assume same for output
                found_channels = True
                
                if checkpoint_channels == 3:
                    print("Using standard 3-channel RGB architecture")
                    config['use_latent_diffusion'] = False
                elif checkpoint_channels == 4:
                    print(f"Using {checkpoint_channels}-channel architecture (VAE latent)")
                    # This is likely a VAE model - adjust settings
                    config['use_latent_diffusion'] = True
                    input_size = input_size // 8
                    print(f"Adjusted input size for latent diffusion: {input_size}x{input_size}")
                else:
                    print(f"Using {checkpoint_channels}-channel architecture")
                break
    ...
    if 'pos_embed' in state_dict and config.get('architecture') == 'dit':
        ...
        num_patches = (input_size // 2) ** 2  # Default patch size 2
        if 'S/1' in model_name:
            num_patches = input_size ** 2  # Patch size 1
        elif 'S/4' in model_name:
            num_patches = (input_size // 4) ** 2  # Patch size 4
        elif 'S/8' in model_name:
            num_patches = (input_size // 8) ** 2  # Patch size 8
        ...
        while sample_index < num_samples:
            ...
            with torch.no_grad():
                ...
                if is_flow_matching:
                    ...
                    r = torch.tensor([radius], device=device, dtype=torch.float32)
