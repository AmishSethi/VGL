...
# Import models for count experiments
...
from flow_matching import create_loss_function, add_flow_matching_args
...
# Add a simple wrapper to convert ImageFolder labels to count values
class ImageFolderWithCount(ImageFolder):
    """Wrapper around ImageFolder that converts class indices to count values."""
    def __init__(self, root, transform=None, count_mapping=None, selected_classes=None, max_samples_per_class=None):
        super().__init__(root, transform)
        # Default mapping: folder names "4", "6", "8", etc. -> count values 4.0, 6.0, 8.0, etc.
        if count_mapping is None:
            # Assume folder names are the count values
            self.count_mapping = {i: float(cls) for i, cls in enumerate(sorted(self.classes))}
        else:
            self.count_mapping = count_mapping
        ...
        if selected_classes is not None:
            ...
            # Update count mapping to only include selected classes
            filtered_count_mapping = {}
            for new_idx, old_idx in enumerate(selected_indices):
                cls_name = self.classes[old_idx]
                filtered_count_mapping[new_idx] = float(cls_name)
            self.count_mapping = filtered_count_mapping
    ...
    def __getitem__(self, index):
        ...
        # Convert label index to count value
        count = self.count_mapping[label]
...
def main(args):
    ...
    if rank == 0:
        ...
        if args.resume_from:
            ...
            # Add "count" to experiment name to distinguish from radius experiments
            experiment_dir = f"{args.results_dir}/{experiment_index:03d}-{model_string_name}-count"  # Create an experiment folder
    ...
    # Create model:
    # Only require image size divisible by 8 for latent diffusion
    ...
    # Model doesn't need num_classes argument (it's ignored in continuous model)
    # Select model based on architecture
    if args.architecture == "dit":
        model = DiT_models[args.model](
            input_size=input_size,
            in_channels=in_channels,
            radius_embedding_type=args.count_embedding_type,  # Use count_embedding_type
            conditioning_method=args.conditioning_method,
            radius_dropout_prob=args.count_dropout_prob,  # Use count_dropout_prob
    ...
    # Use ImageFolderWithCount instead of ImageFolder
    dataset = ImageFolderWithCount(args.data_path, transform=transform, selected_classes=args.selected_classes, max_samples_per_class=args.max_samples_per_class)
    ...
    # Log count mapping
    logger.info(f"Count mapping: {dataset.count_mapping}")
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_idx, (x, c) in enumerate(loader):
                ...
                c = c.to(device)  # This is now count values, not class indices
                ...
                # Pass c (count) instead of y (class) in model_kwargs
                model_kwargs = dict(r=c)  # We reuse the 'r' parameter for counts
                ...
                if torch.isnan(c).any() or torch.isinf(c).any():
                    logger.warning(f"NaN/Inf in count values at step {train_steps}")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--selected-classes", type=float, nargs='+', default=None,
                        help="List of count classes to train on (e.g., 4 6 8 10 for even numbers, or omit for all classes)")
    ...
    parser.add_argument("--count-embedding-type", type=str, choices=["sinusoidal", "rotary", "linear", "single_linear", "raw"], default="sinusoidal",
                        help="Type of count embedding to use (sinusoidal, rotary, linear, single_linear, or raw)") 
    ...
    parser.add_argument("--count-dropout-prob", type=float, default=0.0,
                    help="Count dropout probability for classifier-free guidance (0.0 to disable CFG)")
    parser.add_argument("--null-count", type=float, default=0.0,
                    help="Null count value for unconditional generation in CFG")
