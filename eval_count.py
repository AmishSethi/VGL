...
def detect_and_count_objects(image, min_area=10, max_area=1000):
    """
...
def load_model(config, device):
    ...
    # For SongUNet, always use standard 3-channel RGB architecture
    if config.get('architecture') == 'songunet':
        # Force 3 input and 3 output channels for SongUNet (standard RGB)
        in_channels = 3
        out_channels = 3
        config['use_latent_diffusion'] = False
        print(f"SongUNet detected - using standard 3-channel RGB architecture")
...
def generate_samples(model, diffusion, count_values, config, device, num_samples=3, cfg_scale=1.0):
    ...
    for count in tqdm(count_values, desc="Generating samples"):
        ...
        for i in range(num_samples):
            ...
            if config.get('architecture') == 'songunet':
                # For SongUNet, use the detected input channels from config
                detected_in_channels = config.get('detected_in_channels', 3)
                z = torch.randn(1, detected_in_channels, input_size, input_size, device=device)
            else:
                # For DiT/UNet, use model.in_channels if available
                model_in_channels = getattr(model, 'in_channels', 3)
                z = torch.randn(1, model_in_channels, input_size, input_size, device=device)
            ...
            with torch.no_grad():
                if use_cfg and has_forward_with_cfg:
                    ...
                    r_cond = torch.tensor([count], device=device, dtype=torch.float32)
                    r_uncond = torch.tensor([config['null_radius']], device=device, dtype=torch.float32)  # null_radius used as null_count
                    ...
                    r = torch.tensor([count], device=device, dtype=torch.float32)
                    sample = diffusion.p_sample_loop(
            ...
            if sample.shape[1] == 3:
                ...
                sample = sample.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)[0]
            elif sample.shape[1] == 4:
                # 4-channel output - take first 3 channels as RGB
                sample = sample[:, :3, :, :]  # Take first 3 channels
                sample = torch.clamp(127.5 * sample + 128.0, 0, 255)
                sample = sample.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)[0]
            else:
