...
try:
    from generate_compositional_dataset_coverage import generate_shape_image
except ImportError:
    print("Warning: Could not import generate_shape_image, using simplified ground truth generation")
    def generate_shape_image(radius, position, shape, color_rgb, image_size=64, 
        ...
        center_x = image_size // 2 + position[0]
        center_y = image_size // 2 + position[1]
        
        if shape == 'circle' or shape == 0:
...
def evaluate_properties_comprehensive(image, expected_properties, include_properties, debug=False):
    ...
    # For bright colors (yellow, cyan, magenta), grayscale threshold fails
    # Instead, detect non-white pixels in RGB space
    white = np.array([255, 255, 255], dtype=np.float32)
    diff = np.linalg.norm(image_np.astype(np.float32) - white, axis=2)
    binary = (diff > 30).astype(np.uint8) * 255  # Threshold on distance from white
    ...
    if expected_count and expected_count > 1 and 'count' in include_properties:
        # Use Hough Circle Transform to detect multiple circles
